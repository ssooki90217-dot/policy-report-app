...
11. background(purpose, necessity, current_status, problem_cause), improvement.items[].detail,
    plan[].action — 이 필드들은 각각 **딱 1개의 문장**으로만 작성하고, 한국어 기준 **공백 포함 28자를
    절대 넘기지 않는다** (엄격한 상한선. 28자를 넘기면 모바일 화면에서 3줄 이상으로 보여 반드시 실패로
    간주한다). 문장을 세미콜론이나 접속사로 이어 붙여 2~3문장을 하나의 필드에 담지 않는다. 작성 후
    반드시 스스로 글자 수를 세어보고 28자를 넘으면 더 짧게 줄인다.
...

{
  ...
  "background": {
    "purpose": "목적 (1문장, 28자 이내)",
    "necessity": "필요성/시급성 (1문장, 28자 이내, 없으면 빈 문자열)",
    "current_status": "현황 = 결과·상태 (1문장, 28자 이내, 핵심 수치 하나만)",
    "problem_cause": "문제점 = 원인 (1문장, 28자 이내)",
    "cause_analytic": "분석적 근거 (1문장, 28자 이내, 없으면 빈 문자열)",
    "cause_empathetic": "공감적 근거 (1문장, 28자 이내, 없으면 빈 문자열)"
  },
  "improvement": {
    "criteria": "MECE 분류 기준 (예: 시점 기준: 사전/사중/사후)",
    "items": [
      { "title": "방안 제목", "detail": "구체적 설명 (1문장, 28자 이내)", "related_cause": "관련 원인 요약 (1문장, 20자 이내, 없으면 빈 문자열)" }
    ]
  },
  "plan": [
    { "phase": "단계/시기", "action": "구체적 실행 내용 (1문장, 28자 이내)", "timing": "일정 (없으면 빈 문자열)" }
  ],
  "expected_effect": [
