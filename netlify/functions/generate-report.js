...
//
// 정책기획보고서 생성 함수.
// requirements.md 5장(AI 생성 로직 명세)의 12개 규칙을 시스템 프롬프트에 반영한다.
// ANTHROPIC_API_KEY는 Netlify 사이트의 환경변수로만 설정하고, 프론트엔드에는 절대 노출하지 않는다.
...
11. 한 문장은 2줄(한국어 기준 약 60자) 이내로 작성한다. 길어지면 문장을 나눠 여러 항목으로 분리한다.
12. 사용자가 입력하지 않은 사실(수치, 기관명 등)을 임의로 지어내지 않는다. 부족한 정보는
...

{
  ...
  "background": {
    "purpose": "목적 (1문장)",
    "necessity": "필요성/시급성 (1문장, 없으면 빈 문자열)",
    "current_status": "현황 = 결과·상태 (1~3문장)",
    "problem_cause": "문제점 = 원인 (1~3문장)",
    "cause_analytic": "분석적 근거 (1문장, 없으면 빈 문자열)",
    "cause_empathetic": "공감적 근거 (1문장, 없으면 빈 문자열)"
  },
  "improvement": {
    "criteria": "MECE 분류 기준 (예: 시점 기준: 사전/사중/사후)",
    "items": [
      { "title": "방안 제목", "detail": "구체적 설명 (1~2문장)", "related_cause": "관련 원인 요약 (짧게, 없으면 빈 문자열)" }
    ]
  },
  "plan": [
    { "phase": "단계/시기", "action": "구체적 실행 내용", "timing": "일정 (없으면 빈 문자열)" }
  ],
  "expected_effect": [
...
exports.handler = async (event) => {
  ...
  if (!goal_tobe || !importance || !current_problem || !root_cause) {
    return {
      statusCode: 400,
      body: JSON.stringify({ error: "goal_tobe, importance, current_problem, root_cause는 필수입니다." }),
    };
  }
...
- Q4 (원인 = 문제점): ${root_cause}`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        statusCode: 502,
        body: JSON.stringify({ error: `Anthropic API 오류 (${response.status})`, detail: errBody }),
      };
    }
    ...
    try {
      ...
      return {
        statusCode: 502,
        body: JSON.stringify({ error: "모델 응답을 JSON으로 해석하지 못했습니다.", raw: cleaned }),
      };
    }
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "서버 오류가 발생했습니다.", detail: String(err && err.message ? err.message : err) }),
    };
  }
};
